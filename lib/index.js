  ...
  return schemas.get().then(function(res){
    ...
    var files = results.reduce(function(c, r) {
      ...
      if (file.fileName !== 'DELETE') {
        ...
        if (configuration.markAsTransferedIfFileIsMissing){
          if (configuration.chunkImportantField && configuration.chunkImportantField !== file.fieldName){
            return false;
          }
          return true;
        }
...
function getAllObjects(baseQuery)  {
  ...
  var next = function() {
    ...
    return baseQuery.find({useMasterKey: true}).then(function(r){
      ...
      var lastObject = allObjects[allObjects.length-1];
      process.stdout.write("Gathered Count: " + allObjects.length + ' ' + lastObject.createdAt + '\r');
